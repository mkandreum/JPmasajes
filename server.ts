...
async function startServer() {
  ...
  // Migration: Add blockedHours column to config if it doesn't exist
  try {
    db.exec("ALTER TABLE config ADD COLUMN blockedHours TEXT DEFAULT '[]'");
  } catch (e) { /* column may already exist */ }
  ...
  const getConfig = () => {
    ...
    let blockedHours: { date: string; hours: string[] }[] = [];
    try { blockedHours = row.blockedHours ? JSON.parse(row.blockedHours) : []; } catch (e) { blockedHours = []; }
    return {
...
<body style="margin:0;padding:0;background:#0E1410;font-family:Georgia,serif;">
  ...
  app.put("/api/app-config", requireAdmin, (req, res) => {
    const { bannerUrl, morningHours, afternoonHours, massageTypes, address, logoUrl, logoPosition, tagline, blockedDays, blockedShifts, blockedHours, phone } = req.body;
    const current = getConfig();
    
    db.prepare("UPDATE config SET bannerUrl = ?, morningHours = ?, afternoonHours = ?, massageTypes = ?, address = ?, logoUrl = ?, logoPosition = ?, tagline = ?, blockedDays = ?, blockedShifts = ?, blockedHours = ?, phone = ? WHERE id = 1")
      .run(
        ...
        blockedHours ? JSON.stringify(blockedHours) : JSON.stringify(current.blockedHours || []),
  ...
  app.post("/api/appointments", bookingLimiter, async (req, res) => {
    ...
    db.prepare("INSERT INTO appointments (id, clientName, clientEmail, clientPhone, startTime, endTime, status, eventId, massageType, price, duration) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
      .run(id, clientName, clientEmail, clientPhone, startTime, endTime, 'pending', eventId, massageType, price || null, duration || null);
...
<div style="margin:25px 0;">
  ...
  <p style="font-size:12px;color:#7A7D7B;line-height:1.5;margin-top:10px;">Si no respondes, te enviaremos un recordatorio más tarde.</p>
      ...
      const clientHtml = getHtmlTemplate(
        "Cita Confirmada", 
        `<p>Tu sesión de masaje ha sido reservada con éxito. Estamos deseando recibirte para brindarte la mejor experiencia de relajación.</p>${actionButtons}${addressNotice}`,
        clientName,
        dateStr,
        id,
        massageType
...
<div style="margin:25px 0;">
  ...
  <p style="font-size:12px;color:#7A7D7B;line-height:1.5;margin-top:10px;">Si no respondes, te enviaremos un recordatorio más tarde.</p>
    ...
    const html = getHtmlTemplate(
      "Cita Confirmada",
      `<p>Tu sesión de masaje ha sido reservada con éxito. Estamos deseando recibirte para brindarte la mejor experiencia de relajación.</p>${actionButtons}`,
      appt.clientName,
      dateStr,
      id,
      appt.massageType
